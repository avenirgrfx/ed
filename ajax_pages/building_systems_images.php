    ...
    if ($_POST['mode'] == "add") {
        ...
        foreach ($_POST as $index => $system) {
            ...
            if ($page_no > 0 && $page_no <= 10 && $page_no!="mode") {
                if($ImageFile != ''){
                    $strSQL = "Insert into t_building_system_image (building_id,page_no, building_system_image, system) Values (" . $building_id . "," . $page_no . ",'" . $ImageFile . "'," . $system . ") on duplicate key update building_system_image=values(building_system_image), system=values(system)";
                }else{
                    $strSQL = "Insert into t_building_system_image (building_id,page_no, system) Values (" . $building_id . "," . $page_no . "," . $system . ") on duplicate key update system=values(system)";
                }
                $DB->Execute($strSQL);
            }
        }
...
if (!empty($_POST['strbuilding'])) {
    ...
    <div style="float:left; width:500px;margin-left:30px;">    
        ...
        <?php for ($i = 1; $i <= 10; $i++) { ?>
        <div style="margin:5px;float:left; width:500px;">
            <span style="float:left; width: 65px;">PAGE <?= $i ?>  :</span> 
        <?php if (isset($image["$i"]) && $image["$i"] != "") {
                echo '<input type="button" style="width: 85px; height: 30px;float:left" value="Change" onclick="$(this).next().next().show();$(this).next().hide();$(this).hide();"> <span style="  display: block;float: left;height: 24px;overflow: hidden; padding-left: 5px;width: 190px;" title="' . $image["$i"] . '"> ' . $image["$i"] . '</span><input type="file" name="file2[]" id="image_file_' . $i . '" building_id="' . $building_id . '" style="display:none;float:left;width: 195px;" page_no="' . $i . '" ">';
            } else { ?>
                <input type="file" name="file2[]" id="image_file_<?= $i ?>" building_id="<?= $building_id ?>" page_no="<?= $i ?>"  style="float:left;width:280px;" >
        <?php } ?>
                <span style="float:right"><select id="ddlSystem<?= $i ?>" name="ddlSystem<?= $i ?>" style="width:150px;">
            <?php $System->ListSystems($system[$i]); ?></select></span>
        </div>          
        <?php } ?>

        <div style="float: right;"><input type="button" building_id="<?= $building_id ?>" id="image_save_button" value="Save" style="width:70px;height:30px;" ></div>
    </div>

    <div class="imageNav" style="float:right;margin-right:70px;width:536px;border: 1px solid gray;">
        <span id="building_link_1" style="cursor: pointer; color: #000000; background-color:#CDCDCD; border: 1px solid gray; padding: 1px;" onclick="page_no(1)">PAGE 1</span>
        <span id="building_link_2" style="cursor: pointer; color: #000000; background-color:#CDCDCD; border: none; padding: 1px;" onclick="page_no(2)">PAGE 2</span>
        <span id="building_link_3" style="cursor: pointer; color: #000000; background-color:#CDCDCD; border: none; padding: 1px;" onclick="page_no(3)">PAGE 3</span>
        <span id="building_link_4" style="cursor: pointer; color: #000000; background-color:#CDCDCD; border: none; padding: 1px;" onclick="page_no(4)">PAGE 4</span>
        <span id="building_link_5" style="cursor: pointer; color: #000000; background-color:#CDCDCD; border: none; padding: 1px;" onclick="page_no(5)">PAGE 5</span>
        <span id="building_link_6" style="cursor: pointer; color: #000000; background-color:#CDCDCD; border: none; padding: 1px;" onclick="page_no(6)">PAGE 6</span>
        <span id="building_link_7" style="cursor: pointer; color: #000000; background-color:#CDCDCD; border: none; padding: 1px;" onclick="page_no(7)">PAGE 7</span>
        <span id="building_link_8" style="cursor: pointer; color: #000000; background-color:#CDCDCD; border: none; padding: 1px;" onclick="page_no(8)">PAGE 8</span>
        <span id="building_link_9" style="cursor: pointer; color: #000000; background-color:#CDCDCD; border: none; padding: 1px;" onclick="page_no(9)">PAGE 9</span>
        <span id="building_link_10" style="cursor: pointer; color: #000000; background-color:#CDCDCD; border: none; padding:1px;" onclick="page_no(10)">PAGE 10</span>
        ...
        <div  id="building_image_1" style="width:516px; height:400px;position: relative;padding-left: 4px;margin-left: 5px">
            <img src="<?= URL ?>/uploads/building/<?php if (isset($image["1"])) {
        echo $image["1"];
    } ?>" alt="Image Not Uploaded" style="max-height: 100%;max-width: 100%;width: auto;height: auto;position: absolute; top: 0;bottom: 0;left: 0; right: 0; margin: auto;">
        </div>
        <div  id="building_image_2" style="width:516px; height:400px;position: relative;margin-left: 10px; display:none;">
            <img src="<?= URL ?>/uploads/building/<?php if (isset($image["2"])) {
        echo $image["2"];
    } ?>" alt="Image Not Uploaded" style="max-height: 100%;max-width: 100%;width: auto;height: auto;position: absolute; top: 0;bottom: 0;left: 0; right: 0; margin: auto;">
        </div>
        <div  id="building_image_3" style="width:516px; height:400px;position: relative;margin-left: 10px; display:none;">
            <img src="<?= URL ?>/uploads/building/<?php if (isset($image["3"])) {
        echo $image["3"];
    } ?>" alt="Image Not Uploaded" style="max-height: 100%;max-width: 100%;width: auto;height: auto;position: absolute; top: 0;bottom: 0;left: 0; right: 0; margin: auto;">
        </div>
        <div  id="building_image_4" style="width:516px; height:400px;position: relative;margin-left: 10px; display:none;">
            <img src="<?= URL ?>/uploads/building/<?php if (isset($image["4"])) {
        echo $image["4"];
    } ?>" alt="Image Not Uploaded" style="max-height: 100%;max-width: 100%;width: auto;height: auto;position: absolute; top: 0;bottom: 0;left: 0; right: 0; margin: auto;">
        </div>
        <div  id="building_image_5" style="width:516px; height:400px;position: relative;margin-left: 10px; display:none;">
            <img src="<?= URL ?>/uploads/building/<?php if (isset($image["5"])) {
        echo $image["5"];
    } ?>" alt="Image Not Uploaded" style="max-height: 100%;max-width: 100%;width: auto;height: auto;position: absolute; top: 0;bottom: 0;left: 0; right: 0; margin: auto;">
        </div>
        <div  id="building_image_6" style="width:516px; height:400px;position: relative;margin-left: 10px;display:none;">
            <img src="<?= URL ?>/uploads/building/<?php if (isset($image["6"])) {
        echo $image["6"];
    } ?>" alt="Image Not Uploaded" style="max-height: 100%;max-width: 100%;width: auto;height: auto;position: absolute; top: 0;bottom: 0;left: 0; right: 0; margin: auto;">
        </div>
        <div  id="building_image_7" style="width:516px; height:400px;position: relative;margin-left: 10px; display:none;">
            <img src="<?= URL ?>/uploads/building/<?php if (isset($image["7"])) {
        echo $image["7"];
    } ?>" alt="Image Not Uploaded" style="max-height: 100%;max-width: 100%;width: auto;height: auto;position: absolute; top: 0;bottom: 0;left: 0; right: 0; margin: auto;">
        </div>
        <div  id="building_image_8" style="width:516px; height:400px;position: relative;margin-left: 10px; display:none;">
            <img src="<?= URL ?>/uploads/building/<?php if (isset($image["8"])) {
        echo $image["8"];
    } ?>" alt="Image Not Uploaded" style="max-height: 100%;max-width: 100%;width: auto;height: auto;position: absolute; top: 0;bottom: 0;left: 0; right: 0; margin: auto;">
        </div>
        <div  id="building_image_9" style="width:516px; height:400px;position: relative;margin-left: 10px; display:none;">
            <img src="<?= URL ?>/uploads/building/<?php if (isset($image["9"])) {
        echo $image["9"];
    } ?>" alt="Image Not Uploaded" style="max-height: 100%;max-width: 100%;width: auto;height: auto;position: absolute; top: 0;bottom: 0;left: 0; right: 0; margin: auto;">
        </div>
        <div  id="building_image_10" style="width:516px; height:400px;position: relative; margin-left: 10px; display:none;">
