...
<script>
    function showByCharacter(char){
        ...
        $.get("<?php echo URL ?>ajax_pages/fetch_cpe_node_link.php", {char: char},
            function (data, status) {
                $('#Controls_Container').html(data);
        });
    }
    
     function LoadEquipmentNodeCpeDetails(strSystemID)
            {
                $("#EquipmentNodeSetup_Container_Div").html('Loading...');
                $.get("<?php echo URL ?>ajax_pages/node_cpe_mapping.php",
                        {
...
//       function node_serial1(parent_id,strSystemID){
    ...
    while ($strRsCategory = mysql_fetch_object($strRsCategoryArr)) {
        ...
        while ($strRsSubCat1 = mysql_fetch_object($strRsSubCat1Arr)) {
            ...
            while ($strRsSubCat2 = mysql_fetch_object($strRsSubCat2Arr)) {
                ...
                print "<li><span id='$strRsSubCat2->system_id' style='$strHasNodeStyle' onclick=SystemOptions('" . $strRsSubCat2->system_id . "')>" . $strRsSubCat2->system_name . "</span>&nbsp;&nbsp;<span id='" . $strRsSubCat2->system_id . "'></span><ul>";
                ...
                while ($strRsSubCat3 = mysql_fetch_object($strRsSubCat3Arr)) {
                    ...
                    print "<li style='$strAlternate '>
						<div style='width:330px; float:left;'><span style='$strHasNodeStyle' onclick=SystemOptions('" . $strRsSubCat3->system_id . "')>" . $strRsSubCat3->system_name . "</span> <span style='margin-left:5px; font-size:11px;'>" . $complexity . "</span></div>
						<div style='float:left;'><a href='javascript:void(0)' onclick='LoadEquipmentNodeCpeDetails($strRsSubCat3->system_id)'>Add / Manage Node</a></div> 
