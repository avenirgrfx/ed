...
if($strMultiplier->energy_multiplier && $strMultiplier->energy_multiplier != ""){
...
$utitlity_electric_metered_total = 0;
?>
<table width="100%" border="0" cellspacing="0" cellpadding="0"  >
    ...
    <?php if(mysql_num_rows($strElectricAccountArr)>0) {
    while($strElectricAccount=mysql_fetch_object($strElectricAccountArr)) { 
        ...
        if(isset($output[str_pad($strElectricAccount->month, 2, '0', STR_PAD_LEFT)])){
            $utitlity_electric_metered = $output[str_pad($strElectricAccount->month, 2, '0', STR_PAD_LEFT)];
            $utitlity_electric_metered *= $multiplier; 
            $utitlity_electric_metered_total += $utitlity_electric_metered;
            $diff = number_format((1-($strElectricAccount->consumption/$utitlity_electric_metered))*100, 0);   
        }else{
            $utitlity_electric_metered = 0;
            if($strElectricAccount->consumption == 0){
                $diff = 0;
            }else{
                $diff = -100; 
            }
    ...
    ?>
    <tr>
        <td align="center" valign="middle" style="border:1px solid #CCCCCC;"><?=$strElectricAccount->from?></td>
        <td align="center" valign="middle" style="border:1px solid #CCCCCC;"><?=$strElectricAccount->to?></td>
        <td align="center" valign="middle" style="border:1px solid #CCCCCC;"><?=number_format($utitlity_electric_metered, 0);?></td>
    ...
    <?php 
        if($utitlity_electric_metered_total == 0){
            if($utitlity_electric_consumption_total == 0){
                $total_diff = 0;
            }else{
                $total_diff = -100; 
            }
        } else {
            $total_diff = number_format((1-($utitlity_electric_consumption_total/$utitlity_electric_metered_total))*100, 0); 
        }
    ?>
    
    <tr style="font-weight:bold;">
        <td colspan="2" align="center" valign="middle">&nbsp;</td>
        <td align="center" valign="middle" style="border:1px solid #CCCCCC;"><?=number_format($utitlity_electric_metered_total, 0)?></td>
        <td align="center" valign="middle" style="border:1px solid #CCCCCC;"><?=number_format($utitlity_electric_consumption_total, 0)?></td>
