...
<?php if ($type == 1) { ?>
    ...
    if ($node_id != '') {
        ...
        $strSQL = "Select system_id from t_system where parent_id in(Select system_id from t_system where parent_id in(Select system_id from t_system where parent_id in (Select system_id from t_system )))";
    }
...
<?php } elseif ($type == 2) { ?>
    ...
    if ($strType == 1) {
        ...
        while ($strRsSystems = mysql_fetch_object($strRsSystemsArr)) {
            ...
            if ($strRsSystems->available_system_node_serial <> '') {
                $strSQL = "select device_status_id, UNIX_TIMESTAMP(DATE_FORMAT(synctime,'%Y-%m-%d %H:%i:00')) as synctime, (kwh_diff) as kwh  from `t_$strRsSystems->available_system_node_serial`  where `synctime` >= '$start_date' AND `synctime` <= '$end_date'";
                $consumptionArr = $DB->Returns($strSQL);
                while ($consumption = mysql_fetch_object($consumptionArr)) {
