...
//   calculate e_year and g_year from billing

$strSQL="SELECT CAST(SUBSTRING_INDEX(`from`,'/',1) as UNSIGNED) as month, min(`from`) as date, `to`, sum(consumption) as consumption FROM t_utility_bills B inner join t_utility_account_meters M on B.utility_meter_id = M.utility_meter_number inner join t_utility_accounts A on A.utility_account_id = M.utility_account_id WHERE A.building_id = '$building_id' AND utility_account_type = 1 AND year = '".date('Y')."' group by month";
...
while($strElectricAccount=mysql_fetch_object($strElectricAccountArr)) {
    if($strElectricAccount->consumption > 0){
        $year_consumption += $strElectricAccount->consumption;
        $count++;
    }
}
...
$e_year = 0;
if($count > 0){
    $e_year = $year_consumption/$count;
}

$strSQL="SELECT CAST(SUBSTRING_INDEX(`from`,'/',1) as UNSIGNED) as month, min(`from`) as date, `to`, sum(consumption) as consumption FROM t_utility_bills B inner join t_utility_account_meters M on B.utility_meter_id = M.utility_meter_number inner join t_utility_accounts A on A.utility_account_id = M.utility_account_id WHERE A.building_id = '$building_id' AND utility_account_type = 2 AND year = '".date('Y')."' group by month";
$strElectricAccountArr=$DB->Returns($strSQL);
...
$g_year = 0;
if($count > 0){
    $g_year = $year_consumption/$count;
}

//  end  calculate e_year and g_year from billing 
    ...
    <div>
        ...
        <div style="float:left; border:1px solid #CCCCCC; margin-left:7px; text-align:center;">
            ...
            <div  style="font-size:16px; padding:6px 2px;" id="Target_Site_EUI_ECI_Value">
            	<div class="EUI_Val">
                    <b><strong><?php echo $building_EUI_value; ?></strong>  kBtu/ft<sup>2</sup></b>
                </div>
