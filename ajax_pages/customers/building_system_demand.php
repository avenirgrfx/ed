...
$strSQL="select BS.*, S.system_name, TUV1.variable_unit as variable1_unit, TUV2.variable_unit as variable2_unit, TUV3.variable_unit as variable3_unit, TUV4.variable_unit as variable4_unit from t_building_system BS inner join t_system S on S.system_id = BS.system_id left join t_unit_variable TUV1 on (TUV1.id = BS.variable1_unit) left join t_unit_variable TUV2 on (TUV2.id = BS.variable2_unit) left join t_unit_variable TUV3 on (TUV3.id = BS.variable3_unit) left join t_unit_variable TUV4 on (TUV4.id = BS.variable4_unit)  where BS.id=".$building_system_id;
...
<div>
    <div style="float: left; font-weight: bold; text-decoration: underline; margin: 10px 10px 0 30px; padding: 5px 0px; width: 195px;"><?= $strBuildingSystem->system_name ?></div>
        ...
        <div style="margin: 8px; border: 1px solid #000; height: 505px; text-align: center;position:relative;">
            <div style="display:-moz-box;margin: 8px; border: 1px solid #DDDDDD; height: 320px; width:622px; text-align: center; overflow-x: scroll;">
                <div style="margin: 8px; border: 1px solid #DDDDDD; height: 320px; width:480px;">
                    <object width="430" height="280" data="<?= URL ?>uploads/building/<?= $strBuildingSystem->system_image ?>"></object>
