...
function getParent($strChild) {
    ...
    $DB = new DB;
    $strSQL = "Select parent_id, system_name, system_id, level,exclude_in_calculation from t_system where system_id=$strChild ";
    $strRsGetParentIDArr = $DB->Returns($strSQL);
    while ($strRsGetParentID = mysql_fetch_object($strRsGetParentIDArr)) {
        if($strRsGetParentID->exclude_in_calculation==0){
