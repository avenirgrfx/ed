...
if ($type == 1) {
    ...
    <div style="float:left;">
        ...
        <div style="float:left; width: 204px; font-size: 16px; padding: 13px 1px">
            <div style="float:left; width: 160px; margin-left: 10px; color: #999999; font-size: 18px;">BUILDING NODES: </div><div style="float:right; margin-right: 15px;"><?=sizeof($strBuildingNodesArr);?></div>
            <div style="float:left; width: 160px; margin-left: 10px; color: #999999; font-size: 18px;">ROOM NODES: </div><div style="float:right; margin-right: 15px;"><?=sizeof($strRoomsNodesArr);?></div>
            <div style="float:left; width: 160px; margin-left: 10px; color: #999999; font-size: 18px;">EXTERNAL: </div><div style="float:right; margin-right: 15px;">1</div>
        </div>
                                        ...
                                        if (is_array($level3Arr) && count($level3Arr) > 0) {
                                            foreach ($level3Arr as $val3) {
                                                ...
                                                while ($strRsLevel3 = mysql_fetch_object($strRsLevel3Arr)) {
                                                    $strSQL="select custom_name, system_node_id, available_system_node_serial from t_system_node where delete_flag=0 and system_id in (Select system_id from t_system where parent_id = $strRsLevel3->system_id)";
                                                    $strRsSystemsArr=$DB->Returns($strSQL);
