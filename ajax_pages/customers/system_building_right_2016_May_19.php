...
<?php } else if ($navType == 2){ 
 ...
 <?php  } else if ($navType == 3){ 
    ...
    while($strBuildingSystem = mysql_fetch_object($strBuildingSystemArr)){
        $options .= "<option value='$strBuildingSystem->system_id-#-$strBuildingSystem->system_type-#-$strBuildingSystem->system_name'>$strBuildingSystem->system_name - $strBuildingSystem->system_type</option>";
    }
    $options .= '</select>';
?>  
    <script>
        function getBuildingSystems(ddVal){
            ...
            }else if(ddValArray[1]=="Demand"){
               $.get("<?php echo URL ?>ajax_pages/customers/building_system_demand.php",
            {
                building_id: $('#ddlBuildingForSite').val(),
                system_id: ddValArray[0],
                system_type: ddValArray[1],
