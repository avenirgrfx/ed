...
<script>
    function ShowSystemDetail(id) {
        var ddVal = $('#ddlSystemManage').val();
        var ddValArray = ddVal.split('-#-');
        
        var filename = "";
        if(ddValArray[1]=="SUPPLY"){
            filename = "building_system_detail";
        }else if(ddValArray[1]=="DEMAND"){
            filename = "building_system_demand";  
        }else if(ddValArray[1]=="LIGHTING"){
            filename = "building_system_lighting";
        }
        
        $('#right_side_container').html('Loading...');
        $.get("<?php echo URL ?>ajax_pages/customers/"+filename+".php",
            {
