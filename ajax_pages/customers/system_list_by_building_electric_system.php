		...
		{			
			...
			{   
                if($strRsLevel1->exclude_in_calculation == 1){
                    $strSQL="select available_system_node_serial from t_system_node where delete_flag=0 and building_id=$building_id and system_id in (select system_id from t_system where parent_id in (Select system_id from t_system where parent_id in (Select system_id from t_system where parent_id=$strRsLevel1->system_id)))";
                    ...
                    while($strRsSystemNodes=mysql_fetch_object($strRsSystemNodesArr))
                    {
                     ...
                     $ActualTotalKwh+=floatval($DataVal);
                       // echo "hgfhfg".$ActualTotalKwh;
                    }
                }else{
                    foreach($level2Arr as $val2)
                    {	//secho $strRsLevel1->system_id;	
                        //echo $val2;
                        ...
                        {			
                            ...
                            <div style='float:right; margin-right:20px;' id='totalKwhPercent___".$strRsLevel2->system_id."'>0%</div>
                            ...
                            if(is_array($level3Arr) && count($level3Arr)>0)
                            {
                                ...
                                foreach($level3Arr as $val3)
                                {
                                    ...
                                    $strSQL="Select * from t_system where system_id=$val3 and parent_id=".$strRsLevel2->system_id." and exclude_in_calculation!=1";
                                    ...
                                    while($strRsLevel3=mysql_fetch_object($strRsLevel3Arr))
                                    {
                                        ...
                                        if(is_array($level4Arr) && count($level4Arr)>0)
                                        {
                                            ...
                                            {
                                                $strSQL="Select * from t_system where system_id=$val4 and parent_id=".$strRsLevel3->system_id." and exclude_in_calculation!=1";
                                ...
                                $Percentage_Calculation_IDArr[$strRsLevel2->system_id]=$totalKwh___;
...
<?php
	if(is_array($Percentage_Calculation_IDArr) and count($Percentage_Calculation_IDArr)>0)
	{
        ...
        var finalArray = []
		<?php
		foreach($Percentage_Calculation_IDArr as $Percentage_Calculation_ID => $consumption_value)
		{
            ?>
			var TotalValByID = Math.round(<?php echo ($consumption_value/$totalKwh____)*100 ?>);
