...
<?php if ($type == 1) { ?>
    ...
    if ($node_id != '') {
        ...
        $strSQL = "Select system_id from t_system where parent_id in(Select system_id from t_system where parent_id in(Select system_id from t_system where parent_id in (Select system_id from t_system where  exclude_in_calculation = 0)))";
    }
    ...
    $strSQL = "select custom_name, system_node_id, available_system_node_serial from t_system_node where delete_flag=0 and temperature_humidity=1 and building_id=$building_id and system_id in($strSystemIDs)";
    ...
    if ($strType == 1) {
        ...
        while ($strRsSystems = mysql_fetch_object($strRsSystemsArr)) {
            ...
            if ($strRsSystems->available_system_node_serial <> '') {
                $strSQL = "select device_status_id, UNIX_TIMESTAMP(DATE_FORMAT(synctime,'%Y-%m-%d %H:%i:00')) as synctime, temperature   from  `t_$strRsSystems->available_system_node_serial` where `synctime` >= '$start_date' AND `synctime` <= '$end_date'";
                $consumptionArr = $DB->Returns($strSQL);
                while ($consumption = mysql_fetch_object($consumptionArr)) {
                    $arrTempD.="[" . ( intval($consumption->synctime) * 1000 + $timezoneOffsetMilliSec ) . "," . floatval($consumption->temperature) . "],";
                }
            }
        ...
        while ($strRsSystems = mysql_fetch_object($strRsSystemsArr)) {
            ...
            if ($strRsSystems->available_system_node_serial <> '') {
                $strSQL = "select device_status_id, UNIX_TIMESTAMP(DATE_FORMAT(synctime,'%Y-%m-%d %H:%i:00')) as synctime, humidity   from  `t_$strRsSystems->available_system_node_serial` where `synctime` >= '$start_date' AND `synctime` <= '$end_date'";
                $consumptionArr = $DB->Returns($strSQL);
                while ($consumption = mysql_fetch_object($consumptionArr)) {
                    $arrTempD.="[" . ( intval($consumption->synctime) * 1000 + $timezoneOffsetMilliSec ) . "," . floatval($consumption->humidity) . "],";
                }
            }
...
<?php } elseif ($type == 2) { ?>
    ...
    if ($node_id != '') {
        ...
        $strSQL = "Select system_id from t_system where exclude_in_calculation = 0 and parent_id in(Select system_id from t_system where exclude_in_calculation = 0 and parent_id in(Select system_id from t_system where exclude_in_calculation = 0 and  parent_id in (Select system_id from t_system where exclude_in_calculation = 0 and display_type=$strType)))";
    }
    ...
    if ($strType == 1) {
        ...
        while ($strRsSystems = mysql_fetch_object($strRsSystemsArr)) {
            ...
            if ($strRsSystems->available_system_node_serial <> '') {
                $strSQL = "select device_status_id, UNIX_TIMESTAMP(DATE_FORMAT(synctime,'%Y-%m-%d %H:%i:00')) as synctime, (total_kw) as kwh  from `t_$strRsSystems->available_system_node_serial`  where `synctime` >= '$start_date' AND `synctime` <= '$end_date'";
                $consumptionArr = $DB->Returns($strSQL);
                while ($consumption = mysql_fetch_object($consumptionArr)) {
...
<?php } elseif ($type == 3) { ?>
    ...
    if ($node_id != '') {
        ...
        $strSQL = "Select system_id from t_system where parent_id in(Select system_id from t_system where parent_id in(Select system_id from t_system where  parent_id in (Select system_id from t_system where exclude_in_calculation = 1 and display_type=$strType)))";
    }
    ...
    if ($strType == 1) {
        ...
        while ($strRsSystems = mysql_fetch_object($strRsSystemsArr)) {
            ...
            if ($strRsSystems->available_system_node_serial <> '') {
                ...
                $consumptionArr = $DB->Returns($strSQL);
                while ($consumption = mysql_fetch_object($consumptionArr)) {
                    $arrTempD.="[" . ( intval($consumption->synctime) * 1000 + $timezoneOffsetMilliSec ) . "," . floatval(abs($consumption->kwh)) . "],";
                }
            }
        ...
        while ($strRsSystems = mysql_fetch_object($strRsSystemsArr)) {
            ...
            if ($strRsSystems->available_system_node_serial <> '') {
                ...
                $consumptionArr = $DB->Returns($strSQL);
                while ($consumption = mysql_fetch_object($consumptionArr)) {
                    $arrTempD.="[" . ( intval($consumption->synctime) * 1000 + $timezoneOffsetMilliSec ) . "," . floatval(abs($consumption->kwh)) / 50 . "],";
                }
            }
    ...
    <div style="font-weight: bold; font-size: 16px; margin-left: 10px; margin-bottom: 10px;">REAL-TIME <?php if ($strType == 1) { ?>ELECTRIC<?php } else { ?>NATURAL GAS<?php } ?> DISCONNECT FOR <u><?php echo strtoupper($building_name); ?></u></div>
