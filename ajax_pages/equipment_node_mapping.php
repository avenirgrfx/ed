...
if($strSystemID<>"")
{
	...
	//$strSQL="Select count(1) as SerialNumbers from t_system_node where system_id=$strSystemID and delete_flag=0 and year_of_creation=".date("Y");
	$strSQL="Select count(1) as SerialNumbers from t_system_node where system_id=$strSystemID";
  ...
  $parent_id=$_GET['parent_parent_id'];
    ...
    <script type="text/javascript">
		function LinkSystemNode()
		{
            ...
            var parent_id = $("#val_of_node").attr("parent_id");
			...
			$.post("<?php echo URL?>ajax_pages/system_node_mapping.php",
			{
				...
				//LoadSystemNodeDetails(system_id)
                 //node_serial(parent_id,system_id);
