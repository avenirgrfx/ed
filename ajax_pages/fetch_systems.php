...
if($_POST['type']=='System')
{
    ...
    $System->display_type           =   $_POST['ddlDisplayType'];
	$System->system_name            =   $_POST['txtSystemName'];
	$System->exclude_in_calculation =   $_POST['exclude_in_calculation'];
...
<script>
    ...
    function SystemOptionFetchSystems(strCatID)
    {
        ...
        function (data, status) {
            ...
            $('#ddlDisplayType').val(data[5]);
            if(data[7]==1){
                $('#showInConsumption').prop('checked', false);
                $('#electric_disconnect').prop('checked', true);
            }else{
                $('#showInConsumption').prop('checked', true);
                $('#electric_disconnect').prop('checked', false);
            }
        ...
        $.post("<?php echo URL ?>ajax_pages/fetch_systems.php", 
            {
                ...
                ddlDisplayType: $("#ddlDisplayType").val(),
                txtSystemName : $('#txtSystemName').val(),
                exclude_in_calculation: $('[name=exclude_in_calculation]:checked').val(),
                chkHasWidget : "",                
            },
            function (data, status) {
                showByCharacter("<?=$txtChar?>");
...
<div style="float:left; width:220px;">
    <select id="ddlDisplayType" name="ddlDisplayType" style="width:200px;">  
...
<div style="float:left; width:190px;">
    <input type="radio" value="0" name="exclude_in_calculation" id="showInConsumption" /> <label for="showInConsumption">Show in Consumption</label><br>
    <input type="radio" value="1" name="exclude_in_calculation" id="electric_disconnect" checked /> <label for="electric_disconnect">Electric Disconnect</label>
</div>
<div style="float:left; width:220px;">
...
<?php
    ...
    while($strRsCategory=mysql_fetch_object($strRsCategoryArr))
    {
        ...
        print "<li style='$strDisconnectStyle width:350px; float:left; margin-right: 50px;'><b> <span onclick=SystemOptionFetchSystems('".$strRsCategory->system_id."')>". $strRsCategory->system_name."</span></b><ul>";
        ...
        while($strRsSubCat1=mysql_fetch_object($strRsSubCat1Arr))
        {
            ...
            print "<li style='$strDisconnectStyle'><span onclick=SystemOptionFetchSystems('".$strRsSubCat1->system_id."')>".$strRsSubCat1->system_name."</span></li>";
