...
if(isset($_POST) && !empty($_POST) && $_POST['mode']=='meter'){
    ...
    //print_r($_POST);exit;
    $strSQL="select MAX(STR_TO_DATE(`to`,'%m/%d/%Y')) as last from t_utility_bills B where B.portfolio_status = 1 AND B.utility_meter_id = '".$_POST['meter_id']."'";
    $strLastArr=$DB->Returns($strSQL); 
    $last = '';
    while($strLast=mysql_fetch_object($strLastArr)) {
        $last = $strLast->last;
    }
    
    $strSQL="Select STR_TO_DATE(`from`,'%m/%d/%Y') as start, STR_TO_DATE(`to`,'%m/%d/%Y') as end, consumption, meter_id from t_utility_bills B inner join t_utility_account_meters M on B.utility_meter_id = M.utility_meter_number where B.portfolio_status = 0 AND B.utility_meter_id = '".$_POST['meter_id']."' order by STR_TO_DATE(`from`,'%m/%d/%Y') asc";
    $strBillArr=$DB->Returns($strSQL); 
    //echo date('Y-m-d', strtotime('+1 day', strtotime($last)));exit;
    if(mysql_num_rows($strBillArr) > 0){
        $CreateBill = '<meterData>';
        while($strBill=mysql_fetch_object($strBillArr)) { 
            if($last == '' || $last == $strBill->start || date('Y-m-d', strtotime('+1 day', strtotime($last))) == $strBill->start){
                ...
                echo "There is gap in data between '$last' and '$strBill->start', please fix it.";exit;
            }
        }
        $CreateBill.= '</meterData>';
