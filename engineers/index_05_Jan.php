...
{
	$strSQL="Select * from t_building where site_id=".$strRsSite->site_id;
	$strRsBuildingArr=$DB->Returns($strSQL);
?>
    {
        label: '<?php echo $strRsSite->site_name;?>',
        children: [
            <?php while($strRsBuilding=mysql_fetch_object($strRsBuildingArr)){?>
				{ 
					label: '<?php echo $strRsBuilding->building_name;?>', 
					children: [ 
							{label: '<div style="float:left;">1</div> <div style="float:right;"><a href="#">Edit</a></div><div style="clear:both;"></div>' },
					]				
				},
			<?php }?>           
        ]
 ...
 ul.jqtree-tree .jqtree-element
 {
  ...
  <div id="Menu">
  	<ul>
        ...
        <li id="" style="margin-right:30px;" class="LargeMenu active">Design</li>
        <li id="" style="margin-right:30px;" class="LargeMenu">Controls</li>
     </ul>
...
<div id="ProjectTree_Container" style="display:none;">
    ...
    <hr style="border-bottom:1px #999999 dotted;" />
    <div id="ProjectTree_1" style="display:none;">
    	<?php
        $strSQL="Select * from t_sites order by site_name";
		$strRsSiteArr=$DB->Returns($strSQL);
		while($strRsSite=mysql_fetch_object($strRsSiteArr))
		{
			echo "<div class='site_folder' onclick=ShowBuildingName('".$strRsSite->site_id."')>".$strRsSite->site_name."</div><div id='".$strRsSite->site_id."'></div>";
		}
		?>
    </div>
</div>
