...
function AddWidgetImage(imageName, minScale, maxScale, External_Flag) {
	...
	fabric.Image.fromURL(imageName, function(image) {
	 ...
	 var text1 = new fabric.Text(Temperature_In_Degree, {
	  fontSize: 36,  left: (coord.left+30), top: (coord.top+20), fontFamily:'UsEnergyEngineersDigital', fill:customTempColor, widget_value1:1
	});
	 ...
	 var text2=new fabric.Text(Temperature_Widget_ID,{
							   fontSize:12, left:coord.left, top:coord.top-15, fontFamily:'UsEnergyEngineers', widget_serial:1
							   });
	...
	var group = new fabric.Group([ img1, text1, text2 ], {
		 left: coord.left, top: coord.top, is_widget: true
	});
...
function addAccessors($scope) {
  ...
  $scope.confirmClear = function() {
    if (confirm('Are you sure you want to Close current Workspace? \r\n WARNING: Unsaved work will be Lost!')) {
      canvas.clear();
	  	document.getElementById('ExistingWorkspace_Container').innerHTML='';
		document.getElementById('Open_Workspace_Button').style.display='block';
		document.getElementById('Close_Workspace_Button').style.display='none';	
		document.getElementById('txtControlName').value='';
  ...
  $scope.rasterizeJSON = function() {
	...
	if(document.getElementById('EditWorkspace_ID'))
		edit_id=document.getElementById('EditWorkspace_ID').value;
