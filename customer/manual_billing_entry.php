...
$strSQL="Select site_id, portfolio_status, property_id from t_building where building_id=$BuildingID";
$strRsSiteIDArr=$DB->Returns($strSQL);
while($strRsSiteID=mysql_fetch_object($strRsSiteIDArr))
{
	$strSiteID=$strRsSiteID->site_id;
	$portfolio_status=$strRsSiteID->portfolio_status;
	$property_id=$strRsSiteID->property_id;
}
...
if($_POST)
{
    ...
    if($utility_account_type == 1){
        $meter_type = 'Electric';
    }else if($utility_account_type == 2){
        $meter_type = 'Natural Gas';
    }elseif($utility_account_type == 3){
	...
	{	
		...
		if(is_array($MeterArr) && count($MeterArr)>0)
		{
			foreach($MeterArr as $meter_number)
			{
                ...
                $strSQL="Insert into t_utility_account_meters(utility_account_id,meter_number,meter_id,doc) Values($utility_account_id,'$meter_number','$meter_id',now())";
                $DB->Execute($strSQL);
			}
		}
...
$(document).ready(function(){
	...
	$('#Add_More_Meters').click(function(){
		...
		$('#More_Meter_Container').html(AddedMeters+'<div id="ExtraMeter_Container_'+MeterNumber+'"><div class="clear" style="height:10px;"></div> <div style="float:left; width:150px; font-size:14px;">METER '+MeterNumber+'</div><div style="float:left;"><input type="text" name="txtMeter'+MeterNumber+'" id="txtMeter'+MeterNumber+'"   style="width:200px;"></div></div>');
...
<div style="background-color:#FFFFFF; border-radius:10px; overflow-y:scroll; height:350px;" class="myscroll">
    ...
    <div style="float:left; margin-left:10px; border-left:1px solid #999999; padding-left:10px; padding-top:10px;">
        ...
        <?php
        $strSQL="Select * from t_utility_accounts where building_id=$BuildingID AND utility_account_type = 1";
        $strRsUtilityAccountsArr=$DB->Returns($strSQL);
        $iAccountCtr=1;
        while($strRsUtilityAccounts=mysql_fetch_object($strRsUtilityAccountsArr))
        {
        ...
        <div style="font-size:16px; text-decoration:underline; font-weight:bold; margin-bottom:10px;">EXISTING NATURAL GAS ACCOUNTS</div>
        <?php
        $strSQL="Select * from t_utility_accounts where building_id=$BuildingID AND utility_account_type = 2";
        $strRsUtilityAccountsArr=$DB->Returns($strSQL);
        $iAccountCtr=1;
        while($strRsUtilityAccounts=mysql_fetch_object($strRsUtilityAccountsArr))
        {
        ...
        ?>            
            <div>
                ...
                ACCOUNT# - <?php echo $strRsUtilityAccounts->utility_account_number;?><br />
                <?php while($strRsUtilityMeters=mysql_fetch_object($strRsUtilityMetersArr)){?>
                    <div id="utility_meter_<?php echo $strRsUtilityMeters->utility_meter_number?>">NATURAL GAS METER <?php echo $iMterCtr?> - <?php echo $strRsUtilityMeters->meter_number;?> <a href="javascript:DeleteMeter('<?php echo $strRsUtilityMeters->utility_meter_number?>')">Delete</a></div>
                <?php $iMterCtr++; }?>
            </div>
        ...
        <!--        Water Accounts       -->
        <div style="font-size:16px; text-decoration:underline; font-weight:bold; margin-bottom:10px;">EXISTING WATER ACCOUNTS</div>
        <?php
        $strSQL="Select * from t_utility_accounts where building_id=$BuildingID AND utility_account_type = 3";
        $strRsUtilityAccountsArr=$DB->Returns($strSQL);
        $iAccountCtr=1;
        while($strRsUtilityAccounts=mysql_fetch_object($strRsUtilityAccountsArr))
        {
        ...
        ?>            
            <div>
                ...
                ACCOUNT# - <?php echo $strRsUtilityAccounts->utility_account_number;?><br />
                <?php while($strRsUtilityMeters=mysql_fetch_object($strRsUtilityMetersArr)){?>
                    <div id="utility_meter_<?php echo $strRsUtilityMeters->utility_meter_number?>">WATER METER <?php echo $iMterCtr?> - <?php echo $strRsUtilityMeters->meter_number;?> <a href="javascript:DeleteMeter('<?php echo $strRsUtilityMeters->utility_meter_number?>')">Delete</a></div>
                <?php $iMterCtr++; }?>
            </div>
