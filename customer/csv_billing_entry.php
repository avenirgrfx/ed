    ...
    if($_FILES['csv_file'] && strpos($_FILES['csv_file']['name'],'.csv') !== false){
		...
		if(move_uploaded_file($csvfileTemp, $csvfile)){
			if(file_exists($csvfile)) {
				...
				}else if (strpos($content[0][0],'Gas') !== false){
					$utility_account_type = 2;
                }else if (strpos($content[0][0],'Water') !== false){
                    $utility_account_type = 3;
                }else{
                    $utility_account_type = 0;
                }
...
<div style="background-color:#FFFFFF; border-radius:10px; height:350px;" class="myscroll">
    ...
    <div style="float:left; margin-left:10px; padding-left:10px; padding-top:10px;">
        <div style="width:200px; float: left; width:300px;">
            <div style="font-size:16px; text-decoration:underline; font-weight:bold;  margin-bottom:10px;"><?=$utility_account_type==1?'ELECTRICITY':($utility_account_type==2?'NATURAL GAS':($utility_account_type==3?'WATER':''))?></div>
            ...
            <div style="float:left; width:300px; margin-top: 10px;">
                ...
                <?php foreach($utility_bill_total as $year => $year_total){ ?>
                    <div class="year <?=$year?>" style="display: none;">
                        <div><?=$utility_account_type==1?'ELECTRIC KWH':($utility_account_type==2?'NATURAL GAS THERMS':($utility_account_type==3?'WATER GLN':''))?>: <?= number_format($year_total->kwh,0);?></div>
                        <div><?=$utility_account_type==1?'ELECTRIC COST':($utility_account_type==2?'NATURAL GAS COST':($utility_account_type==3?'WATER COST':''))?>: $<?= number_format($year_total->cost,0);?></div>
                        <div><?=$utility_account_type==1?'ELECTRIC COST/KWH':($utility_account_type==2?'NATURAL GAS COST/THERM':($utility_account_type==3?'WATER COST/GLN':''))?>: $<?= number_format($year_total->avg,2);?></div>
                    </div>
                <?php } ?>
        ...
        <div style="font-size:10px; margin-left:10px; margin-top:10px; width:300px; float:left; border:1px solid #666666;">
            ...
            <table class="year <?=$year;?>" width="100%" border="0" cellspacing="0" cellpadding="0" style="border:1px solid #CCCCCC;" >
                <tr style="background-color:#EFEFEF; font-weight:bold; font-size:9px;">
                    ...
                    <td align="center" valign="middle" style="border:1px solid #CCCCCC;"><?=$utility_account_type==1?'KWH':($utility_account_type==2?'THERMS':($utility_account_type==3?'GLN':''))?></td>
